...
public class Controller03_4 {
  ...
  @GetMapping(value = "step0", produces = "text/palin;charset=UTF-8")
  @ResponseBody
  public String step0(HttpSession session) {
    ...
    // 보통 로그인 과정에서 HttpSession 객체가 준비 될 것이고.
    // 그전에 JSP를 실행하는 과정에서 HttpSession 객체가 생성될 것이다.
    // 따라서 이 메서드 처럼 일부러 HttpSession 객체를 만들게 할 필요는 없다.
    // 다만 @SessionAttributes와 @ModelAttribute를 테스트하기위해
    // 예제를 실행하는 과정에서 HttpSession 객체를 미리 준비할 필요가 있기 때문에,
    // 예제 테스트를 위해 이 메서드를 만든것이다.
    // 이 메서드를 먼저 실행하면 HttpSession 객체를 준비한 후에 순서대로 실행하라!
    return "HttpSession 준비!";
  }
  ...
  public String step4(
    ...
    // 예를들어, "DB에 저장한 후" 세션에서 제거한다.
    return String.format("이름=%s, 나이=%d, 전화=%s 를 DB에 저장!\n", name, age, tel);
  }
