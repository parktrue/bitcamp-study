...
@SessionAttributes({"name", "name2", "age2"})
...
public class Controller03_2 {
  ...
  public String handler4(
      // 세션에는 보관되어 있지만,
      // 현재 페이지컨트롤러의 @SessionAttributes에 지정되지 않은 값 꺼내기
      // => 꺼내지 못한다. name과 age에는 빈 문자열이 저장된다.
      @ModelAttribute("name") String name,
      @ModelAttribute("age") String age,

      // 세션에 보관되어 있고,
      // 현재 페이지컨트롤러의 @SessionAttributes에 지정된 값 꺼내기
      // => 꺼낸다.
    ...
    // 2. @SessionAttributes에 지정된 이름이 아닌경우,
    //  1) 요청 파라미터의 값을 꺼내 넘긴다.
    //  2) 파라미터도 없으면, 빈 문자열을 넘긴다.
    return String.format("name=%s, age=%s, name2=%s, age2=%s, tel2=%s",
