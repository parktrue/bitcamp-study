...
public class DataSource {
  ...
  public Connection getConnection() throws Exception {
    // ConnectionBox를 통해 현재 스레드 보관소에 Connection 객체를 꺼낸다.
    Connection con = connectionBox.get();

    if (con == null) {
      // 현재 스레드에 장착된 DB커넥션 객체가 없다면,

      if (connectionPool.size() > 0) {
        // 커넥션풀에 사용 할 수 있는 Connection 객체가 있다면 꺼낸다.
        con = connectionPool.remove(0);
        System.out.printf("[%s] - DB 커넥션풀에서 꺼냄!\n", Thread.currentThread().getName());
      } else {
        // 커넥션풀에 사용할 Connection 객체가 없다면 새로 생성한다!
  ...
  public void clean() {
    ...
    Connection con = connectionBox.get();
    if (con != null) {
      // 스레드가 사용한 DB커넥션을 다른 스레드에서 사용할 수 있도록 커넥션풀에 저장한다.
      // 다음에 다시 사용해야 하기 때문에 close() 하면 안된다!
      connectionPool.add(con);
      System.out.printf("[%s] - 커넥션풀에 DB커넥션 보관!\n", Thread.currentThread().getName());
