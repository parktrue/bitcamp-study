...
public class BoardNetworkDao implements BoardDao {
  ...
  public void insert(Board board) {
    try {
      ...
      if (response.getStauts().equals(ResponseEntity.ERROR)) {
  ...
  public List<Board> list() {
    try {
      ...
      ResponseEntity response = ResponseEntity.fromJson(in.readUTF());
      if (response.getStauts().equals(ResponseEntity.FAILURE)) {
        throw new RuntimeException(response.getResult());
      }
  ...
  public Board findBy(int no) {
    try {
      ...
      if (response.getStauts().equals(ResponseEntity.ERROR)) {
        throw new RuntimeException(response.getResult());
      } else if (response.getStauts().equals(ResponseEntity.FAILURE)) {
        return null;
      }
