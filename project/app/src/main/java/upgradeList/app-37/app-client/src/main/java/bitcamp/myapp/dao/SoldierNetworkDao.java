...
public class SoldierNetworkDao implements SoldierDao {
  String dataName;
  DataInputStream in;
  DataOutputStream out;

  public SoldierNetworkDao(String dataName, DataInputStream in, DataOutputStream out) {
  ...
  public void insert(Soldier soldier) {
    try {
      out.writeUTF(new RequestEntity().command(dataName + "/insert").data(soldier).toJson());
  ...
  public List<Soldier> list() {
    try {
      ...
      return response.getList(Soldier.class);
  ...
  public Soldier findBy(int no) {
    try {
      ...
      if (response.getStatus().equals(ResponseEntity.ERROR)) {
        throw new RuntimeException(response.getResult());
      } else if (response.getStatus().equals(ResponseEntity.ERROR)) {
        return null;
      }

      return response.getObject(Soldier.class);
  ...
  public int update(Soldier soldier) {
    try {
      // 서버에 요청을 보낸다.
      out.writeUTF(new RequestEntity().command(dataName + "/update").data(soldier).toJson());
