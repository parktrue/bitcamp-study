...
public class App {
    public static void main(String[] args) throws Exception {
        ...
        // 자바 클래스 파일이 갱신되었을때 해당 파일을 자동으로 다시 로딩하도록 설정한다.
        ctx.setReloadable(true);

        // 웹 애플리케이션의  기타 정보 설정
        WebResourceRoot resources = new StandardRoot(ctx);

        // 웹 애플리케이션의 서블릿 클래스 및 기타 파일의 위치정보 등록
        resources.addPreResources(new DirResourceSet(
                resources, // 루트 웹 애플리케이션 정보
                "/WEB-INF/classes", // 서블릿 클래스 파일의 위치정보
                new File("build/classes/java/main").getAbsolutePath(), // 서블릿 클래스 파일이 있는 실제 경로 => bin/main 하위 클래스를 WEB-INF/class와 맵핑
                "/") // 웹 애플리케이션 내부 경로
        );


        // 웹 애플리케션 설정 정보를 웹애플리케이션 환경 정보에 등록
        ...
        System.out.println("엔터를 치면 서버 종료한다!");
